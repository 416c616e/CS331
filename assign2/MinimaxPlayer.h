...
private:
	struct OthelloDecision {
		int col, row;
		OthelloBoard* boardState;
	};

	int maximizingAgent( OthelloBoard* board, int currentDepth, int maxDepth );
	int minimizingAgent( OthelloBoard* board, int currentDepth, int maxDepth );

	int isGameOver( OthelloBoard* board, int currentDepth, int maxDepth );

	int utility( OthelloBoard* board );

	void calculateSuccessors( OthelloBoard* board, char player, OthelloDecision* children, int* countChildren );
